...
get_darwin_internet = function() {
  ...
  if (stringr::str_detect(con_type[1], "Wi-Fi")) {
    ...
    lastTXRate = system(
      "/System/Library/PrivateFrameworks/Apple80211.framework/Resources/airport -I  | awk -F' lastTxRate: '  '/ lastTxRate: / {print $2}'",
      intern = TRUE
    )

    maxRate = system(
      "/System/Library/PrivateFrameworks/Apple80211.framework/Resources/airport -I  | awk -F' maxRate: '  '/ maxRate: / {print $2}'",
      intern = TRUE
    )
    ...
    wifi_string = glue::glue_col(
      "{green {cli::symbol$tick}} {wifi_name} (Strength: {wifi_strength}, lastTXRate: {lastTXRate}, maxRate: {maxRate})"
    )
    # nolint end
  } else {
  ...
  if (stringr::str_detect(con_type[1], "Ethernet")) {
    con_str = glue::glue_col(
      "{wifi_string} | {green {cli::symbol$tick}} ethernet"
    )
  } else {
    con_str = glue::glue_col(
      "{wifi_string} | {red {cli::symbol$cross}} ethernet"
    )
  }
  con_str
}
...
get_linux_internet = function() {
  ...
  if (length(cons) <= 1L) {
    return(crayon::red(cli::symbol$cross))
  }
  ...
  cons = lapply(1:4, function(i) {
    stringr::str_sub(cons[-1], start_locs[i], end_locs[i])
  })
  ...
  if (nrow(wifi) > 0) {
    ...
    wifi_quality = system2(
      "iwconfig",
      args = c(wifi_device, "|", "grep", "-i", "quality"),
      stdout = TRUE
    )
    quality = stringr::str_match(
      wifi_quality,
      "Link Quality=([0-9]*/[0-9][0-9])"
    )[, 2]
    signal = as.numeric(stringr::str_match(wifi_quality, "level=(-[0-9]*)")[,
      2
    ])
    ...
    wifi_string = glue::glue_col(
      "{green {cli::symbol$tick}} {wifi_name} ({wifi_strength})"
    )
  } else {
    wifi_string = glue::glue_col("{red {cli::symbol$cross}} wifi")
  }
...
get_r_sessions = function() {
  if (Sys.info()[["sysname"]] %in% c("Linux", "Darwin")) {
    r_sessions = c(
      system2("ps", args = c("aux", "|", "grep", "rsession"), stdout = TRUE),
...
get_active_rproj = function() {
  ...
  if (isFALSE(rstudioapi::isAvailable())) {
    return(glue::glue("{red(cli::symbol$cross)} ({wd})"))
  }
